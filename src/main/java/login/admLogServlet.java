...
public class admLogServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
            ...
            String sql = "SELECT * FROM admin WHERE Uname='" + username + "' AND Password='" + password + "'";
            ResultSet rs = stmt.executeQuery(sql);
            if (rs.next()) {
                request.getRequestDispatcher("dashboard2").forward(request, response);                // Redirect to instructor dashboard or any other page
            } else {
