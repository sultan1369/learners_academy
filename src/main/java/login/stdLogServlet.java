...
public class stdLogServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
            ...
            String sql = "SELECT * FROM student WHERE Set_Username='" + username + "' AND Set_Password='" + password + "'";
            ResultSet rs = stmt.executeQuery(sql);
            if (rs.next()) {
                request.getRequestDispatcher("dashboard1").forward(request, response);                // Redirect to instructor dashboard or any other page
            } else {
